...
const ChakraList: React.FC<ChakraListProps> = ({ activeChakra, onHover }) => {
  ...
  return (
    <div className="w-full max-w-md ml-auto pr-2 lg:pr-0 z-30 lg:-mt-6">
      <div ref={scrollRef} className="flex flex-col gap-3 h-[86vh] lg:h-[86vh] overflow-y-auto no-scrollbar mask-gradient pb-20 bg-[rgba(10,10,12,0.6)] backdrop-blur-sm rounded-3xl p-4 border border-white/10 shadow-lg font-inter-tight">
        {CHAKRAS.map((chakra, index) => {
          ...
          return (
            <div
              ...
              className={`
                relative group flex flex-col p-4 md:p-5 rounded-2xl cursor-pointer transition-all duration-300 border backdrop-blur-sm
                ${isActive
            ...
            >
              ...
              <div className="flex items-start gap-4">
                {/* Icon Circle */}
                <div className={`
                  w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center text-lg md:text-xl shrink-0 transition-all duration-500
                  ${isActive
